...
def main():
    ...
    目的：
    对比 latent_dim = 6 / 8 / 10 / 12 时，AE+MLP 的验证集和测试集表现。

    为什么要做：
    latent_dim 太小可能丢失对 y 有用的信息；
    latent_dim 太大又可能压缩效果不明显，所以需要用验证集选择。
    """
    base_config = ExperimentConfig()
    set_random_seed(base_config.random_seed)

    output_root = PROJECT_ROOT / "results" / "ablations" / "latent_dim"
